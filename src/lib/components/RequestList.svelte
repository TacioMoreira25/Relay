<script lang="ts">
  ...
  import { IconSearch, IconTrash, IconTerminal, IconFileJson, IconHistory, IconBookmark, IconPlay } from "$lib/components/icons";
  import type { SavedRequestTemplate } from "$lib/types";
  import { invoke } from "@tauri-apps/api/core";

  let { onOpenTemplate = (tpl: SavedRequestTemplate) => {} }: { onOpenTemplate?: (tpl: SavedRequestTemplate) => void } = $props();
...
<div class="flex flex-col h-full bg-zinc-950 text-zinc-200 select-none">
  ...
  <div class="p-2 border-b border-zinc-800/80 bg-zinc-900/40 space-y-2">
    <div class="flex items-center space-x-1 bg-zinc-950 p-0.5 rounded-lg border border-zinc-800/80 text-xs">
      <button
        ...
        class="flex-1 py-1 rounded transition-all flex items-center justify-center space-x-1.5 {relayState.sidebarTab === 'history' ? 'bg-zinc-800 text-zinc-100 font-medium shadow-xs' : 'text-zinc-400 hover:text-zinc-200'}"
      >
        <IconHistory size={13} />
        <span>Histórico</span>
      ...
      <button
        ...
        class="flex-1 py-1 rounded transition-all flex items-center justify-center space-x-1.5 {relayState.sidebarTab === 'collection' ? 'bg-zinc-800 text-zinc-100 font-medium shadow-xs' : 'text-zinc-400 hover:text-zinc-200'}"
      >
    ...
    <div class="flex items-center justify-between px-0.5">
      <span class="text-[11px] text-zinc-400 font-medium">
        {relayState.sidebarTab === 'history' ? 'Tráfego Interceptado' : 'Rotas Salvas para Teste'}
      </span>

      <div class="flex items-center space-x-1.5">
        <label
          class="text-[11px] text-zinc-300 hover:text-white transition-colors px-2 py-0.5 rounded bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 flex items-center space-x-1 cursor-pointer shadow-xs"
        ...
        {#if relayState.sidebarTab === 'history' && relayState.exchanges.length > 0}
          <button
    ...
    <div class="relative flex items-center">
      ...
      <input
        type="text"
        placeholder={relayState.sidebarTab === 'history' ? "Filtrar histórico... (Ctrl+K)" : "Filtrar rotas da coleção..."}
    ...
    {#if relayState.sidebarTab === "history"}
      ...
      {#if relayState.filteredExchanges.length === 0}
        <div class="h-full p-6 flex flex-col items-center justify-center text-center space-y-3 text-zinc-500">
          ...
          <div class="space-y-1">
            <div class="text-xs font-medium text-zinc-300">Sem Tráfego no Histórico</div>
            <p class="text-[11px] text-zinc-500 max-w-[220px] leading-relaxed">
              Envie chamadas para <span class="font-mono text-indigo-400">:{relayState.config.listenPort}</span> ou use a aba <b>Coleção</b> acima.
            </p>
          </div>
        </div>
      {:else}
    ...
    {:else}
      ...
      {#if relayState.filteredTemplates.length === 0}
        <div class="h-full p-6 flex flex-col items-center justify-center text-center space-y-3 text-zinc-500">
          ...
          <div class="space-y-1">
            <div class="text-xs font-medium text-zinc-300">Nenhuma Rota na Coleção</div>
            <p class="text-[11px] text-zinc-500 max-w-[220px] leading-relaxed">
              Clique em <b>Importar Coleção</b> acima para carregar o arquivo <span class="font-mono text-zinc-400">simple-bank.collection.json</span>.
            </p>
          </div>
        </div>
      {:else}
        {#each relayState.filteredTemplates as template (template.id)}
          ...
          >
            ...
            <div class="flex items-center space-x-2">
              <span class="text-[10px] px-1.5 py-0.2 rounded font-mono font-bold border {getMethodBadgeStyle(template.method)}">
                {template.method}
              </span>
